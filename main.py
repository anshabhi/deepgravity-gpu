...
def evaluate():
    ...
    def cpc_from_num(edf, oa2tile, o2d2flow,inpFlows):
        ...
        #    j = 0  
        result_df = [] 
        for i in o2d2flow.keys():
            if j >0:
                for row in o2d2flow[i].keys():
                    dgFlow = o2d2flow[i][row]
                    pop_flows = inpFlows.loc[(inpFlows['geoid_o'] == int(i)) & (inpFlows['geoid_d'] == int(row))]['pop_flows']
                    if len(pop_flows) > 0:
                        orig_flow = pop_flows.values[0]
                    cpc = 2*np.minimum(dgFlow,orig_flow)/(dgFlow+orig_flow)
                    cpc = round(cpc,4)
                    row = {'geoid_o':i,'geoid_d':row,'deeg_gravity_flow':dgFlow,'realFlow':orig_flow,'cpc':cpc}    
                    result_df.append(row)
                    #print(i,row,dgFlow,orig_flow,cpc)
                #j = j + 1
            else:
                break 
        j = 0
    ...
    inpFlows = pd.read_csv('data/new_york/flows.csv')
    cpc_df = cpc_from_num(edf, oa2tile, o2d2flow,inpFlows)
...
test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=args.test_batch_size,pin_memory=True)

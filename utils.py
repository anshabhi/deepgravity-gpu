...
def _is_support_files_computed(db):
    ...
    if os.path.isdir(db+'/processed'):
        base = db + '/processed/'
        print("File Found:",os.path.isfile(base+'tileid2oa2handmade_features.json'))
        #and os.path.isfile(base+'msoa_df_all.csv.zip')
...
def earth_distance(lat_lng1, lat_lng2):
    lat_lng1 = lat_lng1.replace('[', '')
    lat_lng1 = lat_lng1.replace(']', '')
    lat_lng1 = lat_lng1.split(",")

    
    lat1, lng1 = float(lat_lng1[0])*pi/180.0,float(lat_lng1[1])*pi/180.0


    lat_lng2 = lat_lng2.replace('[', '')
    lat_lng2 = lat_lng2.replace(']', '')
    lat_lng2 = lat_lng2.split(",")

    
    lat2, lng2 = float(lat_lng2[0])*pi/180.0,float(lat_lng2[1])*pi/180.0
